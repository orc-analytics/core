ALTER TABLE algorithm_dependency
    ADD COLUMN lookback_gap_count BIGINT NOT NULL DEFAULT 0 CHECK (lookback_gap_count >= 0);
ALTER TABLE algorithm_dependency
    ADD COLUMN lookback_gap_timedelta BIGINT NOT NULL DEFAULT 0 CHECK (lookback_gap_timedelta >= 0);
ALTER TABLE algorithm
    ADD COLUMN self_lookback_gap_count BIGINT NOT NULL DEFAULT 0 CHECK (self_lookback_gap_count >= 0);
ALTER TABLE algorithm
    ADD COLUMN self_lookback_gap_timedelta BIGINT NOT NULL DEFAULT 0 CHECK (self_lookback_gap_timedelta >= 0);
...
search_tree AS (
    ...
    SELECT
        ...
        '0'::VARCHAR AS lookback_gap_count_path,
        '0'::VARCHAR AS lookback_gap_timedelta_path,
        a.self_lookback_gap_count::VARCHAR AS self_lookback_gap_count_path,
        a.self_lookback_gap_timedelta::VARCHAR AS self_lookback_gap_timedelta_path
    FROM algorithm a
    WHERE a.id NOT IN (
    ...
    SELECT
        ...
        st.lookback_gap_count_path || '.' || ad.lookback_gap_count::VARCHAR,
        st.lookback_gap_timedelta_path || '.' || ad.lookback_gap_timedelta::VARCHAR,
        st.self_lookback_gap_count_path || '.' || a.self_lookback_gap_count::VARCHAR,
        st.self_lookback_gap_timedelta_path || '.' || a.self_lookback_gap_timedelta::VARCHAR
...
final_view AS (
    SELECT
        ...
        text2ltree(st.lookback_gap_count_path) AS lookback_gap_count_path,
        text2ltree(st.lookback_gap_timedelta_path) AS lookback_gap_timedelta_path,
        text2ltree(st.self_lookback_gap_count_path) AS self_lookback_gap_count_path,
        text2ltree(st.self_lookback_gap_timedelta_path) AS self_lookback_gap_timedelta_path
    FROM search_tree st
    WHERE
